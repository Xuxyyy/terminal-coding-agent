...
import type {NoticeItem, TaskItem} from '../../ui/events.js';
...
test('a resumed repl shows the summary and keeps the history', async () => {
  const root = workspace();
  const home = process.env.ACC_HOME!;
  ...
  const restored = loadSession(root, null, home);
  const second = fakeModel(() => answer('still here'));
  const two = mount(root, second.choice, restored);
  ...
  const again = loadSession(root, null, home);
  assert.deepEqual(
