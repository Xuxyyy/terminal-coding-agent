...
See also: `agent-loop.md` (who calls `appendTurn`), `permissions.md` (why approvals are
never restored)

Key names, so a search finds this file: `startSession`, `openSession`, `seed`, `appendTurn`,
...
`MAX_TURNS = 20` exists so a confused model cannot burn money forever. It is a **checkpoint,
...
seed the next `appendTurn` writes every one of them to the log a second time. `/resume` does the
...
*message object*, not the string. Without that, `appendTurn` — which `loop.ts` hands the whole
